...
base_ym           = '202106'
# base_ym        = get_base_ym()
colec_path_itg_cd = '10003'  # OnDemand
########################################################################################################################
def selectJobId():
...
with DAG(
    ...
    dag_id='AFOD_002_TCB_CI_TDDC_TDCI_M01_DAG',
    description='처리계DB_수집통합영역데이터적재스케줄링(OnDemand)',
    # schedule='30 04 20 * *',
    schedule='@once',
    ...
    with TaskGroup(group_id='SENSOR_GROUP') as sensor_group:
        for company in companies:
            ExternalTaskSensor(
                task_id=f'S.AFOD_TCB_DC_{company}_TELCO_TXN_M01_DAG',
                external_dag_id=f'AFOD_TCB_DC_{company}_TELCO_TXN_M01_DAG',
