...
def update_data_catalog(schema_list: list=[], **kwargs) -> None:
    ...
    query = f"""
        SELECT
            'cb-prd-trt-db' AS GLUE_DB_NAME,                    -- GLUE DATA CATALOG DB명
...
with DAG(
    ...
    dag_id='AFEC_TCB_CO_TRTS_META_CRAWLER_D01_DAG',
    description='AWS(RDS)_처리계DB메타정보크롤링스케줄',
    schedule='0 16 * * *',
    ...
    crawler = GlueCrawlerOperator(
        task_id='T.GCEC_TRTS_CTLOG_crawler',
        config={
            "Name": "GCEC_TRTS_CTLOG_crawler",
        })

    update_data_catalog_operator = PythonOperator(
        task_id='T.UPDATE_RDS_DATA_CATALOG',
        python_callable=update_data_catalog,
        op_kwargs={'schema_list': ['tcb_dc_db', 'tcb_co_db', 'tcb_dm_db']},
    )
