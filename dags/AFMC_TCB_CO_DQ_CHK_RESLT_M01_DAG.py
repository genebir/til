...
#
# 실제 DAG_IDs:
# AFMC_TCB_CO_KT_DQ_CHK_RESLT_M01_DAG
# AFMC_TCB_CO_SK_DQ_CHK_RESLT_M01_DAG
# AFMC_TCB_CO_LG_DQ_CHK_RESLT_M01_DAG
#
...
for company in companies:
    with DAG(
            ...
            dag_id=f'AFMC_TCB_CO_{company}_DQ_CHK_RESLT_M01_DAG',
            schedule=None,
            tags=['dq', 'monthly', f'{company}'],
            description=f'AWS(GLUE)_{company}_DQ수행결과로그저장스케줄',
            # catchup=False
            catchup=True
    ) as dag:
        start = EmptyOperator(
        ...
        obj = s3_client.get_object(Bucket=f'{s3_meta_bucket}',
                                   Key=f'dataquality/scripts/dq_{company.lower()}_ruleset_info.csv')
        # obj = s3_client.get_object(Bucket='tcb-dev-pl-s3-etl-glue', Key=f'jobs/META-INF/meta_{com.lower()}_ruleset_info.csv')
