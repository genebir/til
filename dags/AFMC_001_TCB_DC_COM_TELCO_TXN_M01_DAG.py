...
def nifi_s3_sensor(table_name: str,
    ...
    return S3KeySensor(
        ...
        bucket_key=f's3://{s3BucketNm}/{company.lower()}/monthly/{table_name}/yearmonth={base_ym}/PERF_{segment_ids[segment_key]}_{table_name[6:].upper()}_{base_date}_{today}.parquet',
        dag=dag
    )
...
for company in COMPANIES.keys():
    # DAG 생성
    with DAG(
        dag_id=f'AFMC_001_TCB_DC_{company}_TELCO_TXN_M01_DAG',
        ...
        crawler = GlueCrawlerOperator(task_id=f"T.GCMC_{company}_DC_CRAWLER",
                                      config={"Name" : f"GCMC_{company}_DC_CRAWLER"}
                                      )
        ...
        trigger_dq = TriggerDagRunOperator(
            ...
            trigger_dag_id=f'AFMC_TCB_CO_{company}_DQ_CHK_RESLT_M01_DAG',
            execution_date=None,
